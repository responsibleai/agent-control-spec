...
The profile is an enforcement path, since its whole purpose is to decide which runes reach the caller. A host MUST NOT feed an `evaluate_only` result into the accounting, because section 20 forbids presenting one as enforcement and a cleared span is enforcement.
