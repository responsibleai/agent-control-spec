...
Every translation failure, and any value the Cedar runtime still rejects, MUST fail closed with `runtime_error:policy_invocation_failed`. A dispatcher MUST NOT evaluate with an empty or partial context in place of one it could not build. The dispatcher error detail names what failed: for a translation failure, the offending key as a dotted path rooted at `context`, without the value; for a schema rejection, the action, and the attribute when the Cedar runtime reports one. The runtime folds every dispatcher error into the fixed verdict of section 12.3, so the detail reaches a host only through a dispatcher the host wraps.
