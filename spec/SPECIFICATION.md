...
A `deny` denies and is terminal for the session. The host MUST stop the stream and MUST NOT release any withheld text. A `deny` carrying an `approval` block is liftable through the host's approval seam (AGENT-HOOKS-0.1 section 9), and resolving it is a host obligation that MUST happen before the outcome reaches the accounting, because a session cannot hold its connection open across an out of band approval. An implementation that receives one unresolved MUST take it at its word and deny, since the decision it carries is `deny` and honoring the seam is the obligation of the host that owns the connection. Withholding the text is the conservative reading, and a host that intended to lift the deny records the lifted outcome instead.
...
A `transform` permits only the substituted value, per section 17. Because the profile holds no text, the host performs the substitution and MUST NOT emit the original, and an implementation MUST record that the stream was modified so the host can report it at settlement. That record is made when the outcome is accepted. It does not depend on the span advancing a task, because a `transform` states that the host will emit a substitute rather than reporting on clearance, so a settlement that ignored one still applicable would under report a rewrite the host is about to perform. An outcome the accounting refuses makes no such record, since a refused span obliges the host to emit nothing at all. A `transform` MUST fail closed wherever the host can no longer alter the text it rewrites. Under `deferred` that is every transform, because the payload was emitted on arrival. Under a withholding level it is any transform recorded once a rune has been released, which includes a session resuming a partially delivered stream, whose starting offset is above zero precisely because that prefix already reached the caller. The bound is the watermark rather than the start of the span because a transform names a node of the policy target and not a rune range, and the profile holds no text, so an implementation cannot know how far below the span the rewritten value reaches. A host evaluating the accumulated prefix has a target covering every rune of the track, including any the watermark already released.

Once a `transform` is recorded for a track, release on that track becomes all or nothing. An implementation MUST NOT report a watermark strictly inside the track, and MUST report the whole track only once every task has cleared everything observed. A substitution replaces the policy target as one value, so the track has no releasable prefix afterwards: its offsets describe the original text while the host holds the replacement, and an offset inside the track names a position in a sequence that no longer exists. Releasing on a partial frontier would emit part of the replacement before a slower task had spoken, and the guarantee that a later refusal still withholds the remainder would not hold, because a rewritten track has no separable remainder.

A second `transform` on a track MUST fail closed with the reserved reason `host_error:transform_conflict`. Two substitutions against the same policy target have no defined composition, and AGENT-HOOKS-0.1 resolves that conflict before a verdict reaches a host, so one arriving at the accounting means neither result is authoritative.

Once a `transform` is recorded for a track, that track MUST accept no further payload.

Taken together these rules mean a `transform` ends its track. It is honored only before anything on that track was released, the track then takes no further payload, and its release is all or nothing. Masking a value mid stream and continuing to stream is outside this profile. The cause is that the outcome carries no length: the host performs the substitution and so knows the replacement's rune count, but does not report it, so an implementation cannot rebase the offsets the substitution shifted. An outcome carrying that count would lift the limit and is the natural extension if it proves too tight. Offsets count the runes the host reported, which are the runes of the original text. A substitution of a different length makes the text the host emits a different sequence from the one those offsets describe, and every later offset on that track would name the wrong position in it. The divergence is permanent and its width is chosen by the replacement value. An implementation is never told that width, holding no text, so it cannot rebase and MUST fail closed instead. The other track is unaffected, since the two are independent offset spaces. A host that must keep streaming after a rewrite starts a new session.
