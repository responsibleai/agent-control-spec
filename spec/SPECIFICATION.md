...
A `transform` is terminal for the session, and an implementation MUST report no watermark for a track once one is recorded. The substitution replaces the policy target with a new whole value. Its runes are not the runes the session counted, so an offset over it names a position in a sequence that no longer exists, and no task on the track evaluated it, so no clearance recorded against the original text authorizes releasing it. Both objections are fatal to an incremental release and neither is repairable by an implementation that holds no text and is never told the replacement's length. The accounting therefore stops, records that the stream was rewritten, and reports the task and the range that produced it. The host evaluates the replacement on the ordinary whole snapshot path of section 18, where every task sees the value actually being emitted, and starts a new session if it must keep streaming.
