...
A `transform` MUST cover every rune the track has observed. A substitution shifts every offset above the span it replaces, so text already observed beyond that span keeps its offset in the accounting while sitting at a shifted position in what the host emits, and a later watermark would name the wrong runes. Requiring the span to reach the observed end removes that tail, and matches what the policy target of a host evaluating its accumulated prefix already contains.
