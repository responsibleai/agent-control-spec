...
For pure decision controls, the examples use `sequential/run_all` so a hard authorization denial is not lifted by another control's approval request. For the classifier pipeline, they use **`sequential/first_deny` with credential screening first**. The preflight covers the retrieved target and the original user prompt added to the provider payload, and rejects hooks outside the selected point. Classifier dispatch itself sends data over the network, so a later classifier must not run after a disclosure denial. A run-all verdict that eventually denies is not enough to prevent that disclosure.
