...
The [document recipe](../recipes/README.md#document-service) makes actual SQLite writes only after authorization and identity-bound approval. Its configurable callback timeout defaults to 300 seconds so an interactive reviewer has time to respond. Rejected, missing, expired or cancelled approval leaves the database unchanged. Its scripted test reviewer is not a production approver. The host must authenticate reviewers, display the exact action, bind responses to its context identity, enforce expiry/replay rules and implement durable suspension if needed. Preserve additional CLI/framework prompts.
