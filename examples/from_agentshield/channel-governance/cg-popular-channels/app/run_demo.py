#!/usr/bin/env python3
"""Deterministic ACS runtime demo for the ported Popular channels channel governance policy."""
...
SECRET_PATTERNS = [
    ...
    ("telegram_token", re.compile(r"\b\d{7,10}:[A-Za-z0-9_-]{20,}\b", re.IGNORECASE)),
    ("gcp_bearer", re.compile(r"ya29\.[A-Za-z0-9._-]{20,}", re.IGNORECASE)),
    ("whatsapp_session_secret", re.compile(r"(noiseKey|advSecretKey|signedIdentityKey)", re.IGNORECASE)),
...
async def main() -> None:
    ...
    print("ACS Popular channels channel governance demo")
    print(f"manifest={MANIFEST.relative_to(MANIFEST.parents[4])}\n")

    await check(control, {"tool_call": {"name": "channel.send_message", "args": {"destination": "slack:engineering", "operation": "send", "message": "routine update"}}}, "allowlisted send requires approval (escalate)")
    await check(control, {"tool_call": {"name": "channel.send_message", "args": {"destination": "outside-destination", "operation": "send", "message": "routine update"}}}, "non-allowlisted destination (deny)")
    await check(control, {"tool_call": {"name": "channel.send_message", "args": {"destination": "slack:engineering", "operation": "delete", "message": "routine update"}}}, "dangerous operation (deny)")
    await check(control, {"tool_call": {"name": "channel.send_message", "args": {"destination": "slack:engineering", "operation": "send", "message": "xoxb-1234567890-token"}}}, "secret-bearing message (deny)")
