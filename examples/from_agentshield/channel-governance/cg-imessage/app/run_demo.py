#!/usr/bin/env python3
"""Deterministic ACS runtime demo for the ported iMessage channel governance policy."""
...
SECRET_PATTERNS = [
    ("bluebubbles_credential", re.compile(r"(bluebubbles|imessage).*?(password|token|secret)\s*[:=]\s*\S+", re.IGNORECASE)),
...
async def main() -> None:
    ...
    print("ACS iMessage channel governance demo")
    print(f"manifest={MANIFEST.relative_to(MANIFEST.parents[4])}\n")

    await check(control, {"tool_call": {"name": "imessage.send_message", "args": {"destination": "family", "operation": "send", "message": "routine update"}}}, "allowlisted send requires approval (escalate)")
    await check(control, {"tool_call": {"name": "imessage.send_message", "args": {"destination": "outside-destination", "operation": "send", "message": "routine update"}}}, "non-allowlisted destination (deny)")
    await check(control, {"tool_call": {"name": "imessage.send_message", "args": {"destination": "family", "operation": "attachment_write", "message": "routine update"}}}, "dangerous operation (deny)")
    await check(control, {"tool_call": {"name": "imessage.send_message", "args": {"destination": "family", "operation": "send", "message": "bluebubbles password=demo-secret"}}}, "secret-bearing message (deny)")
