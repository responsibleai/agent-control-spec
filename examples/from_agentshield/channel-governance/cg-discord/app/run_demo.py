#!/usr/bin/env python3
"""Deterministic ACS runtime demo for the ported Discord channel governance policy."""
...
async def main() -> None:
    ...
    print("ACS Discord channel governance demo")
    print(f"manifest={MANIFEST.relative_to(MANIFEST.parents[4])}\n")

    await check(control, {"tool_call": {"name": "discord.send_message", "args": {"destination": "engineering", "operation": "send", "message": "routine update"}}}, "allowlisted send requires approval (escalate)")
    await check(control, {"tool_call": {"name": "discord.send_message", "args": {"destination": "outside-destination", "operation": "send", "message": "routine update"}}}, "non-allowlisted destination (deny)")
    await check(control, {"tool_call": {"name": "discord.send_message", "args": {"destination": "engineering", "operation": "delete", "message": "routine update"}}}, "dangerous operation (deny)")
    await check(control, {"tool_call": {"name": "discord.send_message", "args": {"destination": "engineering", "operation": "send", "message": "MAAAAAAAAAAAAAAAAAAAAAAA.AAAAAA.AAAAAAAAAAAAAAAAAAAAAAAAAAA"}}}, "secret-bearing message (deny)")
