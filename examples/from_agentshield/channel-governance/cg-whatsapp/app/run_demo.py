#!/usr/bin/env python3
"""Deterministic ACS runtime demo for the ported WhatsApp channel governance policy."""
...
SECRET_PATTERNS = [
    ("whatsapp_session_secret", re.compile(r"(noiseKey|advSecretKey|signedIdentityKey|signalIdentity)", re.IGNORECASE)),
...
async def main() -> None:
    ...
    print("ACS WhatsApp channel governance demo")
    print(f"manifest={MANIFEST.relative_to(MANIFEST.parents[4])}\n")

    await check(control, {"tool_call": {"name": "whatsapp.send_message", "args": {"destination": "+15550202020", "operation": "send", "message": "routine update"}}}, "allowlisted send requires approval (escalate)")
    await check(control, {"tool_call": {"name": "whatsapp.send_message", "args": {"destination": "outside-destination", "operation": "send", "message": "routine update"}}}, "non-allowlisted destination (deny)")
    await check(control, {"tool_call": {"name": "whatsapp.send_message", "args": {"destination": "+15550202020", "operation": "credential_store", "message": "routine update"}}}, "dangerous operation (deny)")
    await check(control, {"tool_call": {"name": "whatsapp.send_message", "args": {"destination": "+15550202020", "operation": "send", "message": "advSecretKey leaked"}}}, "secret-bearing message (deny)")
