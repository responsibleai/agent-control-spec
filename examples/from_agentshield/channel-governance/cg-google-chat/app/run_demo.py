#!/usr/bin/env python3
"""Deterministic ACS runtime demo for the ported Google Chat channel governance policy."""
...
async def main() -> None:
    ...
    print("ACS Google Chat channel governance demo")
    print(f"manifest={MANIFEST.relative_to(MANIFEST.parents[4])}\n")

    await check(control, {"tool_call": {"name": "google_chat.send_message", "args": {"destination": "spaces/engineering", "operation": "send", "message": "routine update"}}}, "allowlisted send requires approval (escalate)")
    await check(control, {"tool_call": {"name": "google_chat.send_message", "args": {"destination": "outside-destination", "operation": "send", "message": "routine update"}}}, "non-allowlisted destination (deny)")
    await check(control, {"tool_call": {"name": "google_chat.send_message", "args": {"destination": "spaces/engineering", "operation": "admin", "message": "routine update"}}}, "dangerous operation (deny)")
    await check(control, {"tool_call": {"name": "google_chat.send_message", "args": {"destination": "spaces/engineering", "operation": "send", "message": "ya29.AAAAAAAAAAAAAAAAAAAAAAAA"}}}, "secret-bearing message (deny)")
