#!/usr/bin/env python3
"""Deterministic ACS runtime demo for the ported Signal channel governance policy."""
...
SECRET_PATTERNS = [
    ("signal_account_command", re.compile(r"(register|link device|unregister|change number)", re.IGNORECASE)),
...
async def main() -> None:
    ...
    print("ACS Signal channel governance demo")
    print(f"manifest={MANIFEST.relative_to(MANIFEST.parents[4])}\n")

    await check(control, {"tool_call": {"name": "signal.send_message", "args": {"destination": "+15550101010", "operation": "send", "message": "routine update"}}}, "allowlisted send requires approval (escalate)")
    await check(control, {"tool_call": {"name": "signal.send_message", "args": {"destination": "outside-destination", "operation": "send", "message": "routine update"}}}, "non-allowlisted destination (deny)")
    await check(control, {"tool_call": {"name": "signal.send_message", "args": {"destination": "+15550101010", "operation": "register", "message": "routine update"}}}, "dangerous operation (deny)")
    await check(control, {"tool_call": {"name": "signal.send_message", "args": {"destination": "+15550101010", "operation": "send", "message": "please register this Signal device"}}}, "secret-bearing message (deny)")
