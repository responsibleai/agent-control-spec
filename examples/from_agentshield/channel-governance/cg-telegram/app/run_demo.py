#!/usr/bin/env python3
"""Deterministic ACS runtime demo for the ported Telegram channel governance policy."""
...
async def main() -> None:
    ...
    print("ACS Telegram channel governance demo")
    print(f"manifest={MANIFEST.relative_to(MANIFEST.parents[4])}\n")

    await check(control, {"tool_call": {"name": "telegram.send_message", "args": {"destination": "@ops_bot", "operation": "send", "message": "routine update"}}}, "allowlisted send requires approval (escalate)")
    await check(control, {"tool_call": {"name": "telegram.send_message", "args": {"destination": "outside-destination", "operation": "send", "message": "routine update"}}}, "non-allowlisted destination (deny)")
    await check(control, {"tool_call": {"name": "telegram.send_message", "args": {"destination": "@ops_bot", "operation": "delete", "message": "routine update"}}}, "dangerous operation (deny)")
    await check(control, {"tool_call": {"name": "telegram.send_message", "args": {"destination": "@ops_bot", "operation": "send", "message": "123456789:AAAAAAAAAAAAAAAAAAAA"}}}, "secret-bearing message (deny)")
