...
# --------------------------------------------------------------------------- #
# AGENT-HOOKS-0.1 migration (DRAFT — see docs/AGENT-HOOKS-MIGRATION.md)
#
# The hook contract — InterventionPoint, Decision, EnforcementMode, Verdict,
# Transform, Evidence, approval seam, canonical JSON, context identity — now
# lives in the standalone `agent-hooks` package. ACS re-exports under its
# existing names so downstream callers see no source change. The ACS-owned
# concepts (PolicyInput, annotators, manifest, dispatchers, runtime_error:*)
# remain defined below.
#
# Once agent-hooks 0.1 is published, the local definitions of the re-exported
# names below this block are DELETED and only these aliases remain. They are
# kept for now so the existing 992 conformance tests continue to pass while
# agent-hooks is iterated privately.
# --------------------------------------------------------------------------- #
try:
    from agent_hooks import (  # noqa: F401
        Decision as _AH_Decision,
        EnforcementMode as _AH_EnforcementMode,
        Evidence as _AH_Evidence,
        HookPoint as _AH_HookPoint,
        Transform as _AH_Transform,
        Verdict as _AH_Verdict,
        canonical_json as _ah_canonical_json,
        context_identity as _ah_context_identity,
    )
    from agent_hooks import HookBlocked as AgentControlBlocked  # noqa: F401
    from agent_hooks import HookSuspended as AgentControlSuspended  # noqa: F401

    _AGENT_HOOKS_AVAILABLE = True
except ImportError:  # pragma: no cover — bootstrap path before agent-hooks is installable
